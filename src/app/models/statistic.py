...
class Statistic(Modelable):
    ...
    def from_row(cls, row: List[Any]) -> Self:
        ...
        return (
            ...
            .with_total_sales(total_sales)
            .with_inventory_items(inventory_items)
            .with_menu_items(menu_items)
            .with_total_orders(total_orders)
        )
    ...
    def top_menu_items(cls) -> List[Self]:
        cur = cls._db.execute(
            "SELECT d.name AS name, COUNT(do.dish_id) AS amount FROM Dishes d JOIN Dishes_Orders do ON do.dish_id = d.id JOIN Orders o ON o.id = do.order_id WHERE do.is_healthy = 0 AND d.is_predef = 1 GROUP BY d.name ORDER BY amount DESC LIMIT 3;"
        )
        return cur.fetchall()
